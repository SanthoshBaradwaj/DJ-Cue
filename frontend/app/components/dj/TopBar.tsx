...
function StatusPill({ status }: { status: FeedStatus }) {
  ...
  return (
    ...
    >
      ...
      <span className="text-[13px] font-bold tracking-[0.16em]" style={{ color }}>
        {label}
      </span>
    </div>
  );
...
function MicroStat({ value, label }: { value: string; label: string }) {
  return (
    <div className="flex flex-col items-end leading-tight">
      <span className="tnum text-base font-semibold text-chalk/85">{value}</span>
      <span className="text-[12px] uppercase tracking-[0.14em] text-mist/60">{label}</span>
    </div>
  );
}
      ...
      <div className="flex flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:gap-4 sm:px-5 sm:py-3.5 xl:gap-6 xl:px-8">
        ...
        <div className="flex items-center justify-between gap-3 sm:justify-start">
          <div className="flex items-baseline gap-[1px]">
            <span className="text-3xl font-black tracking-[-0.04em]" style={{ color: "var(--color-cue-1)" }}>
              DJ
            </span>
            <span className="text-3xl font-black tracking-[-0.04em] text-chalk">-CUE</span>
          </div>
